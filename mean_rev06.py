...
request_token = "ul7fcj5zDM6zzkqYYnLkSWya2FlHPmM7" #Extract request token from the redirect url obtained after you authorize yourself by loggin in
Data = kite.generate_session(request_token, api_secret=api_secret)

#create kite trading object
#kite.set_access_token(Data["access_token"])
kite.set_access_token("Gwjp1KMsdfhPhiQIVjpIOzzj3q5Hvnel") 
...
def bbands(close,period,bar):
    upper,middle,lower=talib.BBANDS(close.values,timeperiod=period,nbdevup=bar, nbdevdn=bar,matype=0)
...
slowperiod=10
sl_per=10/100
#start = datetime.date(2020,9,6)
#end = datetime.date(2021,9,6)
...
#for t in tickers:
    ...
    #data_capture2[t] = fetchOHLC(t,"15minute",200)
...
d1 = fetchOHLC("SHREECEM","15minute", 200)
...
data["K"],data["D"]=ta.STOCH(data["high"],data["low"],data["close"],fastk_period=14, slowk_period=3, slowk_matype=0, slowd_period=3, slowd_matype=0)
...
for i,value in data.iloc[(sma+5):].iterrows():
    ...
    if(data["signal"][loc-1]==0 
       and data["RSI"][loc-2]>rsi_up and data["K"][loc-2]<80 and data["D"][loc-2]<80 
       and data["ACCDist"][loc-1]<data["ACCDist"][loc-2]):
        entry_s=entry_s+1
        data["signal"][loc]=-1
        data["Price"][loc]=(data["close"][loc])-((data["close"][loc]*commission)/100)
        sl_price_sell = data["high"][loc-2]+(data["high"][loc-2]*sl_per)
        
    elif(data["signal"][loc-1]==-1 and data["high"][loc]>sl_price_sell):
        data["signal"][loc]=0
        data["Price"][loc]=(data["close"][loc])+((data["close"][loc]*commission)/100)
        data["sqoff"][loc]=1
       ...
       and data["RSI"][loc-2]<rsi_down and data["K"][loc-2]>20 and data["D"][loc-2]>20 
       and data["ACCDist"][loc-1]>data["ACCDist"][loc-2]):
        entry_b=entry_b+1
        data["signal"][loc]=1
        data["Price"][loc]=(data["close"][loc])+((data["close"][loc]*commission)/100)
        sl_price_buy = data["low"][loc-2]-(data["low"][loc-2]*sl_per)
        
    elif(data["signal"][loc-1]==1 and data["low"][loc]<sl_price_buy):
        data["signal"][loc]=0
        data["Price"][loc]=(data["close"][loc])-((data["close"][loc]*commission)/100)
        data["sqoff"][loc]=1
